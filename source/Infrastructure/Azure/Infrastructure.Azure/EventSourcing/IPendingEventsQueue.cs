...
namespace Infrastructure.Azure.EventSourcing
{
    ...
    public interface IPendingEventsQueue
    {
        ...
        Task GetPendingAsync(string partitionKey, Action<IEnumerable<IEventRecord>, TableContinuationToken> successCallback, Action<Exception> exceptionCallback);
        ...
        /// </param>
        /// <param name="exceptionCallback">The callback used if there is an exception that does not allow to continue.</param>
        Task DeletePendingAsync(string partitionKey, string rowKey, Action<bool> successCallback, Action<Exception> exceptionCallback);

        /// <summary>
        /// Gets the list of all partitions that have pending unpublished events.
        /// </summary>
        /// <returns>The list of all partitions.</returns>
        Task<IEnumerable<string>> GetPartitionsWithPendingEvents();
